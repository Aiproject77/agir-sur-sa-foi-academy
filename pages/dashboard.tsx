...
import { PUBLIC_COURSES } from "../lib/courses";
...
export default function Dashboard() {
  ...
  function canAccess(i: number): boolean {
    if (i === 0) return true;
    return !!user?.progress?.[PUBLIC_COURSES[i - 1].id]?.completedAt;
  }

  const totalChapters = PUBLIC_COURSES.reduce((s, c) => s + c.chapters.length, 0);
  const completedChapters = PUBLIC_COURSES.reduce((s, c) => s + (user?.progress?.[c.id]?.completedChapters?.length || 0), 0);
  const completedCourses = PUBLIC_COURSES.filter((c) => user?.progress?.[c.id]?.completedAt).length;
    ...
    <>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: "2.5rem" }}>
          {[
            { label: t.coursesCompleted, value: `${completedCourses} / ${PUBLIC_COURSES.length}` },
